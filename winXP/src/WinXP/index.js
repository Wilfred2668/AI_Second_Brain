...
function WinXP() {
  ...
  function onDoubleClickIcon(component, iconData = null) {
    // Handle Upload Files icon - trigger file input
    ...
    // Handle dynamic file icons
    if (iconData && iconData.isFile) {
      const file = iconData.fileData;
      const appName = fileService.getAppForFile(file);
      const appSetting = appSettings[appName];
      
      if (appSetting) {
        // Create custom app setting with file data
        const fileAppSetting = {
          ...appSetting,
          header: {
            ...appSetting.header,
            title: fileService.getWindowTitle(file),
          },
          // Pass file data to the component via injectProps
          injectProps: {
        ...
        dispatch({ type: ADD_APP, payload: fileAppSetting });
      }
      return;
    }
    
    // Handle regular app icons
      ...
      if (result.success) {
        ...
        dispatch({
          ...
          payload: {
            ...
            injectProps: {
              message: `File "${result.filename}" uploaded successfully!\n\n${result.processed ? 'File has been processed and added to AI memory.' : 'File saved.'}`,
            },
          },
        });
      } else {
  ...
  // Combine static icons with dynamic file icons
  const allIcons = [...state.icons, ...fileIcons];
